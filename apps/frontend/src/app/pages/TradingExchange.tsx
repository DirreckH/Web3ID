...
import { type TradeInstrument, listTradeInstruments } from "../lib/dataGateway";
...
const shellClass =
  "min-h-full bg-gray-50 pb-24 lg:rounded-[34px] lg:border lg:border-white/70 lg:bg-white/92 lg:pb-32 lg:shadow-[0_32px_84px_rgba(15,23,42,0.08)]";
...
export function TradingExchange() {
  ...
  const instruments = useMemo(
    () =>
      allTokens.filter((instrument) => {
        const haystack = `${instrument.symbol} ${instrument.name} ${instrument.description}`.toLowerCase();
    ...
    [allTokens, assetFilter, productType, query],
  );
  ...
  if (!selected) {
    return (
      <div className={shellClass} data-testid="trade-page">
            ...
            <>
              <div className="mb-6 flex items-center gap-3">
                <div className="relative flex-1">
                  ...
                  <input
                    className="w-full rounded-2xl border border-gray-200 bg-gray-50 py-3.5 pl-12 pr-4 text-sm outline-none focus:border-blue-500 focus:bg-white"
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        ...
                        <div className="mb-2 flex flex-wrap items-center gap-2">
                          <span className={`rounded-full px-2.5 py-1 text-[10px] font-semibold ${eligibilityClasses.badge}`}>{eligibility.badgeLabel}</span>
                          <span className="text-[11px] font-medium text-gray-500">{eligibility.listSummary}</span>
                        </div>
            ...
            {positions.map((position, index) => {
              ...
              return (
                <motion.div
                  key={position.id}
                  animate={{ opacity: 1, y: 0 }}
                  className="rounded-3xl border border-gray-100 bg-white p-5 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)]"
                  initial={{ opacity: 0, y: 20 }}
  ...
  return (
    <div className={`bg-gradient-to-b from-gray-50 to-white ${shellClass}`} data-testid="trade-page">
      <motion.div animate={{ opacity: 1, y: 0 }} className="sticky top-0 z-20 border-b border-gray-100/50 bg-white/90 px-6 pb-4 pt-12 backdrop-blur-xl" initial={{ opacity: 0, y: -20 }}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex-1 text-center">
            ...
            <div className={`mx-auto mt-3 max-w-xl rounded-2xl border px-4 py-3 text-left shadow-sm ${selectedEligibilityClasses.panel}`} data-testid="trade-eligibility-summary">
              <div className="flex flex-wrap items-center gap-2">
                <span className={`rounded-full px-2.5 py-1 text-[10px] font-semibold ${selectedEligibilityClasses.badge}`}>{selectedEligibility.badgeLabel}</span>
              ...
              <div className={`mt-1 text-[11px] ${selectedEligibilityClasses.detail}`}>还差哪一步: {selectedEligibility.missingStep ?? "当前无需补充步骤"}</div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
            <div className="scrollbar-hide flex items-center gap-2 overflow-x-auto">
              {(["1m", "5m", "15m", "1h", "4h", "1d"] as const).map((entry) => (
                <button
                  key={entry}
                  className={`rounded-xl px-4 py-2 text-xs font-semibold ${timeframe === entry ? "bg-blue-600 text-white shadow-lg shadow-blue-600/30" : "text-gray-500 hover:bg-gray-100"}`}
      ...
      <div className="px-6">
        <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
          ...
          {side === "buy" ? (
            <div className={`mb-5 rounded-2xl border px-4 py-4 shadow-sm ${selectedEligibilityClasses.panel}`} data-testid="trade-buy-eligibility-card">
              ...
              <div className={`mt-2 text-[11px] ${selectedEligibilityClasses.detail}`}>下一步: {selectedEligibility.nextAction}</div>
            </div>
          ) : null}
            ...
            {orderType === "limit" ? (
              <div>
                ...
                <div className="relative">
                  <input
                    className="w-full rounded-2xl border-2 border-gray-200 bg-gray-50 px-4 py-4 text-sm font-semibold outline-none focus:border-blue-500 focus:bg-white"
          ...
          <div className="mb-5 rounded-2xl border border-gray-200 bg-gray-50 px-5 py-4 text-xs">
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-center gap-2 text-gray-500">
                <Wallet className="h-4 w-4" />
                <div>
                  <div className="font-bold">可用余额</div>
                  <div className="mt-1 text-[11px] text-gray-400">APY {selected.apy}% · 可购 {selected.available} 份</div>
                </div>
              </div>
              <div className="text-right">
                <div className="text-sm font-bold text-gray-900">10,000.00 USDT</div>
                <div className="mt-1 text-[11px] text-gray-400">{selected.location}</div>
              </div>
            </div>
          </div>
