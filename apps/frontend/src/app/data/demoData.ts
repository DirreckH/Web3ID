...
const tradeUnderlyings: TradeUnderlying[] = [
  {
    id: "nyc",
    symbol: "NYC",
...
export function createPriceSeries(asset: ChartableAsset, timeframe: ChartTimeframe) {
  ...
  return Array.from({ length: points }, (_, index) => {
    ...
    return { id: `${asset.id}-${timeframe}-${index}`, time: `${index + 1}`, close, volume: Math.round(asset.volume24h / points + (Math.cos(index + seed) + 1) * 12_000) };
  });
}
...
export function createRecentTrades(asset: Pick<TradeAssetQuote, "id" | "price">) {
  ...
  return Array.from({ length: 10 }, (_, index) => {
    ...
    return { id: `${asset.id}-trade-${index}`, price: Number(price.toFixed(asset.price < 1 ? 5 : 2)), amount, time: `09:${String(index * 3).padStart(2, "0")}:12`, side: multiplier > 0 ? "buy" : "sell" };
  });
}
