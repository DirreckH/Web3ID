...
async function waitForDemoReady() {
  ...
  await waitForHttp("http://127.0.0.1:4200/health");
  await waitForHttp("http://127.0.0.1:4300/health");
...
const demoArgs = isWindows ? ["/c", "pnpm", "demo:stage3"] : ["demo:stage3"];
...
try {
  await freePorts([3000, 4100, 4200, 4300, 8545]);
