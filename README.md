...
- EVM controller 已升级为 registry-based 支持，覆盖 `Ethereum Mainnet / BNB Chain / Arbitrum One / Base / OP Mainnet`。
...
- 前端 EVM 演示当前已支持本地 Foundry / Anvil（`31337`）与 `BNB Chain (56)` 作为运行时链目标。
...
> 当前前端运行时支持本地 Foundry / Anvil 开发链 `31337`，以及用于 BNB 主网演示的 `56`。默认本地开发路径仍然保持 `31337`。
...
这轮多链能力的主要落点是 backend / SDK / analyzer / verifier / audit，不代表前端已经为每个 family 都提供完整的 wallet connect 与签名 UI。当前已经明确落到前端 EVM 演示面的，是本地 Foundry / Anvil（`31337`）与 `BNB Chain (56)`。这个边界是有意保持的。
